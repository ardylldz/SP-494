...
async def run():
    ...
    config.read(os.path.expanduser("~/Masaüstü/drone1_config.ini"))
    ...
    try:
        ...
        await asyncio.sleep(22)  # Takeoff sonrası drone havalansın
        ...
        print(f"{CYAN}[SHM] Temizleniyor...{ENDC}")
        telemetry_shm.close()
        if created_new_shm:
            try:
                ...
                pass
        print(f"{GREEN}[SHM] Kapandı ve silindi.{ENDC}")
